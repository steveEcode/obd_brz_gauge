// 曲线报警阈值设置页 (曲线页上滑进入)
//  - 针对「当前曲线显示的数据项」设置报警阈值(每项独立, 存 NVS chart_alarm)
//  - 滑条量程 = 该数据项自然量程; 拉到最大档 = OFF(关闭报警)
//  - 值 >= 阈值时, 各页该项数值变红(见 disp_item_set_value_color)
//  - 任意方向手势返回曲线页 (见 ui_event_chart_alarm_background)

#include "../ui.h"
#include "bsp_obd_dsp/nvs_storage.h"

#define CHART_ALARM_OFF 32767   // 与 nvs 约定: 32767 = 关闭
...
void ui_ScreenPageChartAlarm_screen_init(void)
{
    ...
    lv_obj_set_style_bg_color(ui_ScreenPageChartAlarm, lv_color_hex(0x000000), LV_PART_MAIN);
    ...
    // 滑条: 量程 = [nmin, nmax+1], nmax+1 档 = OFF
    ...
    lv_obj_clear_flag(s_alarm_slider, LV_OBJ_FLAG_GESTURE_BUBBLE);   // 拖动不触发页面返回手势
