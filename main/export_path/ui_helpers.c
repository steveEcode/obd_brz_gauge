...
/*
 * 模拟器只记录七个正常仪表页面。
 * 设置、设备信息、警告和配置页面不会成为下次启动页。
 */
static void simulator_remember_last_gauge_page(
    lv_obj_t **target
)
{
    uint8_t page;

    if (target == &ui_ScreenPageTemp) {
        page = 0;
    } else if (target == &ui_ScreenPageInfo) {
        page = 1;
    } else if (target == &ui_ScreenPageOilPressure) {
        page = 2;
    } else if (target == &ui_ScreenPageNeedle) {
        page = 3;
    } else if (target == &ui_ScreenPageGear) {
        page = 4;
    } else if (target == &ui_ScreenPageRpm) {
        page = 5;
    } else if (target == &ui_ScreenPageSpeed) {
        page = 6;
    } else {
        return;
    }
    ...
    if (cfg.default_page == page) {
        return;
    }

    cfg.default_page = page;
    nvs_cfg_set(&cfg);
}
#endif
...
void _ui_screen_change(lv_obj_t ** target, lv_scr_load_anim_t fademode, int spd, int delay, void (*target_init)(void))
{
    if(*target == NULL)
        target_init();
#ifdef SIMULATOR_BUILD
    simulator_remember_last_gauge_page(target);
#endif
