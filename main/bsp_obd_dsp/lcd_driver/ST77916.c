...
/* v2 init sequence (default for V1 board; new board B under V3).
   Used by every build except V2_NEW, so guard against -Wunused-const-variable. */
...
#if CONFIG_OBD_HW_VERSION_V2_NEW
/* v1 init sequence (new board A, selected when CONFIG_OBD_HW_VERSION_V2_NEW=1).
   Source: tools/te.cpp lcd_init_cmd_v1 (identical to tools/scr_st77916.h lcd_init_cmd_v1). */
static const st77916_lcd_init_cmd_t vendor_specific_init_new[] = {
...
#if CONFIG_OBD_HW_VERSION_V3_NEW
static const st77916_lcd_init_cmd_t vendor_specific_init_new[] = {
  {0xF0, (uint8_t []){0x28}, 1, 0},
  {0xF2, (uint8_t []){0x28}, 1, 0},
  {0x73, (uint8_t []){0xF0}, 1, 0},
...
int QSPI_Init(void){
  ...
  if (ret == ESP_OK) {
    ESP_LOGI(TAG_LCD, "Register 0x04 data: %02x %02x %02x %02x", register_data[0], register_data[1], register_data[2], register_data[3]);
  } else {
  ...
  // Each hardware version has its own dedicated init table (see the three
  // version-guarded vendor_specific_init_new[] definitions above). Apply it
  // UNCONDITIONALLY — do not rely on the register read below, which may fail
  // or return an unknown panel ID (falling back to the driver's default table
  // previously made V3's panel not display at all).
#if CONFIG_OBD_HW_VERSION_V2_NEW
  vendor_config.init_cmds = vendor_specific_init_new;   // new board A: v1 sequence (0xF0=0x08)
  vendor_config.init_cmds_size = sizeof(vendor_specific_init_new) / sizeof(st77916_lcd_init_cmd_t);
  ESP_LOGI(TAG_LCD, "Hardware V2: applying v1 init sequence (0xF0=0x08).");
#elif CONFIG_OBD_HW_VERSION_V3_NEW
  vendor_config.init_cmds = vendor_specific_init_new;   // new board B: v2 sequence (0xF0=0x28)
  vendor_config.init_cmds_size = sizeof(vendor_specific_init_new) / sizeof(st77916_lcd_init_cmd_t);
  ESP_LOGI(TAG_LCD, "Hardware V3: applying v2 init sequence (0xF0=0x28).");
#else
  vendor_config.init_cmds = vendor_specific_init_new;   // V1 Waveshare: v2 sequence (0xF0=0x28)
  vendor_config.init_cmds_size = sizeof(vendor_specific_init_new) / sizeof(st77916_lcd_init_cmd_t);
  ESP_LOGI(TAG_LCD, "Hardware V1: applying v2 init sequence (0xF0=0x28).");
#endif
