...
/* 广播包和 presence 包结构由 espnow_protocol.h 统一定义。 */
...
static void handle_rx(const uint8_t *mac, const uint8_t *data, int len) {
    if (len == (int)sizeof(espnow_obd_packet_t)) {
        ...
        if (p->magic == ESPNOW_PROTOCOL_MAGIC && p->version == ESPNOW_PROTOCOL_VERSION) handle_obd(p);
        ...
        if (pr->magic == ESPNOW_PROTOCOL_MAGIC && pr->version == ESPNOW_PROTOCOL_VERSION) handle_presence(mac, pr);
    }
}
...
static void slave_presence_task(void *arg) {
    espnow_presence_t pr = { .magic = ESPNOW_PROTOCOL_MAGIC, .version = ESPNOW_PROTOCOL_VERSION };
    for (;;) {
